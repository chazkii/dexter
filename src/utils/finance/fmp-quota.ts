...
/**
 * Daily call-quota tracker for the Financial Modeling Prep (FMP) free tier.
 *
 * The FMP free plan allows 250 calls/day. This counter persists to
 * `.dexter/fmp-quota.json`, resets at midnight UTC, and surfaces a warning
 * once usage crosses 80% so the agent can prefer other sources before the
 * quota is exhausted.
 */
...
function defaultQuotaPath(): string {
  return dexterPath('fmp-quota.json');
}
